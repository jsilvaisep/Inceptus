...
<div class="empresa-completa-container">
    ...
    <div class="empresa-header">
        <?php if (!empty($company['LOGO_URL'])): ?>
            <img src="<?= htmlspecialchars($company['LOGO_URL']) ?>" alt="Logo de <?= htmlspecialchars($company['COMPANY_NAME']) ?>">
        <?php endif; ?>
            ...
            <?php foreach ($products as $product): ?>
                <div class="produto-item">
                    ...
                    <a href="?page=produtocompleto&id=<?= urlencode($product['PRODUCT_ID']) ?>">
                        <img src="<?= htmlspecialchars($product['IMG_URL']) ?>" alt="<?= htmlspecialchars($product['PRODUCT_NAME']) ?>" class="produto-img">
                    </a>
                    <h3><?= htmlspecialchars($product['PRODUCT_NAME']) ?></h3>
                    <p><strong>Rating:</strong> <?= renderStars($product['PRODUCT_RANK']) ?> (<?= htmlspecialchars($product['PRODUCT_RANK']) ?>)</p>
